...
export default function PriceComponent(props) {
    ...
    const getProdPrice = (prodName, cat) => 
        props.price || [getProdPriceCat0, getProdPriceCat1Or3, getProdPriceCat2, getProdPriceCat1Or3][cat](prodName, cat)
    const getProdPriceCat0 = prodName => 
        _.get(props.prods, [props.prodName, 'dados', props.size, 'valor_unitario]'])
            || ('R$ ' + Products.categories[0][prodName].size
                        .find((item) => item.name === props.size).price
                        .toLocaleString('pt-br', { minimumFractionDigits: 2 }))
    const getProdPriceCat1Or3 = (prodName, cat) => 
        _.get(props.prods, [props.prodName, 'valor_unitario'])
            || 'R$ ' + Products.categories[cat][prodName].price.toLocaleString('pt-br', { minimumFractionDigits: 2 })    
    const getProdPriceCat2 = prodName => 
        _.get(props.prods, [props.prodName, 'dados', props.subtype, 'valor_unitario'])
    ...
    if (props.show) {
        ...
        const price = getProdPrice(props.prodName, getProdCategory(props.prodName))
        return (
            <div key="input-group" 
                        style={{
                            margin: '0 1em', 
