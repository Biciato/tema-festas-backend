...
<div className="products" key={props.prod}>
    <PriceComponent
        key="price-cpt"
        show={true}
        ...
        uppercase={true}
        onPriceChange={price =>
            props.onPriceChange(props.prodName, props.size || null, price)
        }
        priceInput={props.prod.tipo_categoria !== 2}
    />
    {(props.prod.tipo_categoria &&
        Object.keys(props.prod.dados)
            .filter(type => type !== "valor_unitario")
            .map((type, idx) => {
                if (props.prod.tipo_categoria === 2) {
                    return (
                        <div
                            ...
                            <QuantityComponent key={type + idx}
                                ...
                                line={true}
                                onQtyChange={subtype =>
                                    props.onSubtypeChange(
                                        props.prodName,
                                        subtype
                                    )
                                }/>
                            <PriceComponent
                                price={props.prod.dados[type].valor_unitario}
                                key={type + idx + "price"}
                    ...
                    return Object.keys(props.prod.dados[type]).map(
                        (subtype, idx) => (
                            <div style={{
                                    backgroundColor:
                                        idx % 2 === 0 ? "white" : "#F8F8F8"
